...
import { ISynthraV3Pool, IWETH9, MockTimeSwapRouter02, TestERC20 } from '../typechain'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [ISynthraV3Pool, ISynthraV3Pool, ISynthraV3Pool]
    ...
    const pools = poolAddresses.map((poolAddress) => new ethers.Contract(poolAddress, ISynthraV3PoolABI, wallet)) as [
      ISynthraV3Pool,
      ISynthraV3Pool,
      ISynthraV3Pool
    ]
  ...
  let pools: [ISynthraV3Pool, ISynthraV3Pool, ISynthraV3Pool]
  ...
  describe('#exactInput', () => {
    ...
    it('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestSynthraV3Callee')
