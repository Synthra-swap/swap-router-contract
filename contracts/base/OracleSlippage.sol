...
import '@synthra-swap/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@synthra-swap/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@synthra-swap/v3-periphery/contracts/libraries/Path.sol';
import '@synthra-swap/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@synthra-swap/v3-core/contracts/interfaces/ISynthraV3Pool.sol';
import '@synthra-swap/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(ISynthraV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (ISynthraV3Pool pool) {
        pool = ISynthraV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            ISynthraV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = ISynthraV3Pool(pool).slot0();
            }
